...
def main():
    ...
    #the thing below works too
    #LE = client.commandExecute('getCalendar')
    ...
    pr = json.loads(sharks)

    current_price = client.commandExecute('getSymbol',pr)
    from pandas import DataFrame
    curry = DataFrame(list(current_price.items()), columns = ['Something 1','Something 2'])
    cur_pr_sell = curry['Something 2'][1]['ask']
    cur_pr_buy = curry['Something 2'][1]['bid']
    #print(type(cur_pr))
    with open('Current Price.txt','w') as output:
        output.write('This is the sell price ' + str(cur_pr_sell))
        output.write('\n' + 'This is the buy price ' + str(cur_pr_buy))

    #Getting server time
    teem = client.commandExecute('getServerTime')
    tim = DataFrame(list(teem.items()), columns = ['Something 1','Something 2'])
    toime = tim['Something 2'][1]['time']
    
    with open('D:/Trading/XTB/Demo Trading Strat/Demo Live V1/Current Time.txt','w') as output:
        output.write(str(toime))
        


    #Volume
    balance = open('D:\Trading\XTB\Demo Trading Strat\Demo Live V1\My Balance.txt')
    bae = balance.readlines(0)[0]
    bon = float(bae)
    ...
    buye = json.dumps(barg)
    buy = json.loads(buye)
    client.commandExecute('tradeTransaction',buy)
    ...
    #I am going to try to get the code to get me my account balance
    #My_Account_Balance = sclient.subscribeBalance()
