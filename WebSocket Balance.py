...
class XTBClient:
    def __init__(self, host='wss://ws.xtb.com/demo', port=5124, stream_port=5125):
        """
    ...
    def get_chart_range_request(self, symbol, start, end, period=1440, ticks=0):
...
test = XTBClient(host='xapia.x-station.eu', port=5124, stream_port=5125)


test.login(user_id=11182830, password="password")


my_bal = str(test.get_balance()['balance'])

with open('My Balance.txt','w') as output:
    output.write(str(my_bal))
